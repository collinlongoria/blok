...
const ShaderModuleEntry &ShaderSystem::loadModule(const std::string &glslPath, vk::ShaderStageFlagBits stage) {
    ...
    ent.spirv = compileShader(loadFile(glslPath), stage);
...
std::string ShaderSystem::loadFile(const std::string &path) {
...
std::vector<uint32_t> ShaderSystem::compileShader(const std::string &source, const vk::ShaderStageFlagBits stage) {
    ...
    const auto spvVersion = glslang::EShTargetSpv_1_5;
    ...
    if (!shader.parse(&DefaultTBuiltInResource, 460, false, EShMsgDefault)) {
